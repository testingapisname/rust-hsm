...
use simple_asn1::{ASN1Block, ASN1Class, OID, BigInt, BigUint, to_der};
...
pub fn generate_csr(
    ...
    info!("Generating CSR for key '{}' on token '{}' in slot {}", key_label, label, usize::from(slot));
    ...
    // Build the CSR
    let (tbs_der, signature_algorithm_oid) = build_tbs_certificate_request(&session, public_key_handle, &key_type, subject_dn)?;
    ...
    println!("Certificate Signing Request generated for key '{}'", key_label);
...
fn build_tbs_certificate_request(
    ...
    let cert_req_info = ASN1Block::Sequence(0, vec![
...
fn parse_subject_dn(dn: &str) -> anyhow::Result<ASN1Block> {
    ...
    for component in dn.split(',') {
        ...
        let atv = ASN1Block::Sequence(0, vec![
...
fn get_public_key_info(
    ...
    match *key_type {
        KeyType::RSA => {
            ...
            let rsa_key = ASN1Block::Sequence(0, vec![
            ...
            let algorithm = ASN1Block::Sequence(0, vec![
            ...
            let spki = ASN1Block::Sequence(0, vec![
                algorithm,
        ...
        KeyType::EC => {
            ...
            let ec_params_asn = ec_params_block.first().ok_or_else(|| anyhow::anyhow!("Failed to parse EC params"))?;
...
fn build_certificate_request(
    ...
    let tbs_block = tbs_block.first().ok_or_else(|| anyhow::anyhow!("Failed to parse TBS"))?;
    
    // Build AlgorithmIdentifier for signature
    let sig_algorithm = ASN1Block::Sequence(0, vec![
    ...
    let cert_req = ASN1Block::Sequence(0, vec![
        tbs_block.clone(),
        sig_algorithm,
