...
pub fn generate_random(
    ...
    info!("Generating {} random bytes using HSM RNG in slot {}", bytes, usize::from(slot));
    ...
    session.generate_random_slice(&mut random_bytes)
    ...
    if let Some(output_path) = output {
        if hex {
            // Write as hex string
            let hex_string = random_bytes.iter()
                .map(|b| format!("{:02x}", b))
                .collect::<String>();
            fs::write(output_path, hex_string)
                .with_context(|| format!("Failed to write hex output to: {}", output_path.display()))?;
            println!("Random bytes (hex) written to: {}", output_path.display());
        } else {
            // Write as binary
            fs::write(output_path, &random_bytes)
                .with_context(|| format!("Failed to write output to: {}", output_path.display()))?;
            println!("Random bytes (binary) written to: {}", output_path.display());
        }
