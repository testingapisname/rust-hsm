...
fn test_tui_info_command_softhsm() -> Result<()> {
    ...
    assert!(
        !app.command_output.is_empty(),
        "Info command should produce output"
    );

    // Check for SoftHSM-specific content
    let output_text = app.command_output.join("\n");
    assert!(
        output_text.contains("SoftHSM") || output_text.contains("Implementation of PKCS#11"),
        "Output should contain SoftHSM information"
    );

    // Check status message
    assert!(
        app.status.contains("successfully"),
...
fn test_tui_list_slots_command_softhsm() -> Result<()> {
    ...
    assert!(
        !app.command_output.is_empty(),
        "List-slots command should produce output"
    );

    // Check for expected slot listing format
    let output_text = app.command_output.join("\n");
    assert!(
        output_text.contains("=== Initialized Slots ==="),
        "Output should contain initialized slots section"
    );
    assert!(
        output_text.contains("=== All Slots ==="),
        "Output should contain all slots section"
    );

    // Should have substantial output (many lines)
    assert!(
        app.command_output.len() > 10,
        "Should have multiple lines of slot information"
    );
...
fn test_tui_info_command_kryoptic() -> Result<()> {
    ...
    let output_text = app.command_output.join("\n");
    assert!(
        output_text.contains("Kryoptic") || output_text.contains("PKCS#11"),
...
fn test_tui_scrolling_with_real_data() -> Result<()> {
    ...
    assert!(
        app.command_output.len() > 15,
        "Should have substantial slot data for scrolling"
    );
    ...
    if app.command_output.len() > 10 {
        assert!(
            ...
            "Should scroll down with real data"
        );
    }
    ...
    app.scroll_up();
    assert!(
        app.scroll_offset <= app.command_output.len(),
        "Scroll offset should stay within bounds"
    );
